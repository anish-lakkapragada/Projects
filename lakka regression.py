...
class ExpBaseRegression() :
    ...
    def evaluate(self, x_test, y_test):
        y_pred = ExpBaseRegression.predict(self, x_test)
        avg_error = np.mean(np.abs(y_pred - y_test))
        return 1 - avg_error/np.mean(y_test)

base = ExpBaseRegression(learning_rate = 0.1)
base.fit(X[10:], y[10:], epochs = 3000)
print("Evaluation : ", base.evaluate(X[:10], y[:10]))